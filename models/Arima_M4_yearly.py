...
def naive2_forecast(series, prediction_length=6):
    return series[-prediction_length:]

# Script modification for the M4 dataset
def process_m4_series(train, test, prediction_length=6):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'../test_results/ARIMA/M4/Yearly/Yearly_forecast_next_{prediction_length}_steps_scaled.png')
    plt.show()
...
# Example usage
# Load the M4 dataset
train_data_path = '../dataset/m4/Yearly-train.csv' 
test_data_path = '../dataset/m4/Yearly-test.csv'  # Replace with your file path
...
with open('../test_results/ARIMA/M4/Yearly/ARIMA_M4_Scaled_Forecast_Results.pkl', 'wb') as file:
